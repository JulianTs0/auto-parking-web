...
export class AuthDataSource implements AuthDataSourceI {
    ...
    async auth(dto: AuthReq): Promise<AuthRes> {
        return await this.client.get("/auth", undefined, dto.authorization);
    }
    ...
    async changePassword(dto: EditPasswordReq): Promise<void> {
        ...
        return await this.client.patch("/auth/password", payload, session.getAccessToken());
    }
    ...
    async registerEmployee(dto: RegisterEmployeeReq): Promise<void> {
        ...
        return await this.client.post("/web/auth/employee/register", body, session.getAccessToken());
    }

    async getOwnerRequests(dto: GetOwnerRequestReq): Promise<GetOwnerRequestRes> {
        const { session, ...payload } = dto; 
        return await this.client.get("/web/auth/owner/requests", payload, session.getAccessToken());
    }

    async acceptOwnerRequest(dto: AcceptOwnerRequestReq): Promise<void> {
        const { session, ...body } = dto;
        return await this.client.patch("/web/auth/accept/owner", body, session.getAccessToken());
    }

    async requestOwnerUpgrade(dto: RequestOwnerUpgradeReq): Promise<void> {
        const { session, ...body } = dto;
        return await this.client.patch("/web/auth/request/upgrade/owner", body, session.getAccessToken());
    }

    async upgrade(dto: UpgradeToOwnerReq): Promise<void> {
        const { session, ...body } = dto;
        return await this.client.patch("/web/auth/upgrade", body, session.getAccessToken());
    }
}
